...
- **URL 목록으로 위키 등록**: "위키 문서 등록" 바로 아래 섹션. "제목" 한 줄, "URL" 한 줄을 번갈아
  텍스트박스에 붙여넣고 "URL 위키에 업로드"를 누르면, 각 URL을 스크랩해서(`requests`+`BeautifulSoup`,
  `article`/`main`/`#content` 우선) 개별 위키 페이지로 업로드함 (`wiki_upload.upload_sites_to_wiki()`).
  페이지 제목은 `YYYY-MM-DD 제목`, 본문엔 출처 URL·수집 시각·분류 태그가 자동으로 붙음 (분류 기본값
  `ITS동향`, 직접 입력창에서 수정 가능). 빈 줄과 `#`으로 시작하는 줄은 무시됨
